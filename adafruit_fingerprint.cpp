...
uint8_t Adafruit_Fingerprint::loadChar(uint16_t idNum)//∂¡≥ˆƒ£∞Â ∑≈µΩBUF1
{
	uint8_t packet[] = {FINGERPRINT_LOADCHAR,0x01,idNum>>8,idNum};
...
uint8_t Adafruit_Fingerprint::setSysPara(uint8_t dat)//…Ë÷√ƒ£øÈ≤®Ãÿ¬ 
{
	uint8_t packet[] = {FINGERPRINT_SETSYSPARA, 0x04, dat};
	writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);
	uint8_t len = getReply(packet);
...
uint8_t Adafruit_Fingerprint::upChar(void)//…œ¥´ƒ£∞Â
{
	...
	writePacket(theAddress, FINGERPRINT_COMMANDPACKET, sizeof(packet)+2, packet);// ∑¢ÀÕ…œ¥´√¸¡Ó
	uint16_t len = 0;

	len = getReply(packet); //”¶¥ø™ º…œ¥´
